...
feature_extractor = ViTImageProcessor.from_pretrained("nlpconnect/vit-gpt2-image-captioning", load_in_4bit=True)
model = VisionEncoderDecoderModel.from_pretrained("nlpconnect/vit-gpt2-image-captioning", load_in_4bit=True)
    ...
    try:
        ...
        for frame in sorted(annotations[video].keys()):
            ...
            if ((len(batch_id) >= batchsize) or (frame == num_frames-1)) and len(batch_id) > 0:
                inputs = feature_extractor(batch_image, return_tensors="pt").to("cuda", torch.float16)
